...
public class TaskServica {
    ...
    public ApiResponse createTask(TaskDto taskDto) {
        ...
        RoleName responsibleUserRoleName = responsibleUser.getRole().getRoleName();
        //VAZIFANI YUKLASH
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        User systemUser = (User) authentication.getPrincipal();
        if (systemUser.getRole().getRoleName().name().equals("DIRECTOR")) {
            if (responsibleUserRoleName.name().equals("MANAGER")) {
        ...
        if (systemUser.getRole().getRoleName().name().equals("HR_MANAGER")) {
            if (responsibleUserRoleName.name().equals("MANAGER") || responsibleUserRoleName.name().equals("EMPLOYEE")) {
                newTask.setResponsibleUser(responsibleUser);
                newTask.setTaskStatus(TaskStatus.NEW);
                taskRepository.save(newTask);
                sendEmailAboutTask(responsibleUser.getEmail(),newTask.getId());
        ...
        return new ApiResponse("Vazifa " + systemUser.getFirstName() + " " + systemUser.getLastName() + " tomonidan "
                + responsibleUserRoleName.name() + " " + responsibleUser.getFirstName() + " " + responsibleUser.getLastName() + "ga yuklatildi", true);
    }
